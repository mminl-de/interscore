	...
	switch (ev) {
		...
		case MG_EV_CLOSE: {
			...
			if (clients.first->con == con) {
				...
				for (Client *c = clients.first; c->next; c = c->next) {
					if (c->next->con == con) {
...
int main(int argc, char *argv[]) {
	...
	for (int i = 1; i < argc; i++) {
		if (!strcmp(argv[i], "--disable-obs"))
			...
			if (argc != i + 2) die("Syntax: --url-obs needs a url...");
			url_obs = malloc(strlen(argv[i + 1]) + 1);
			strcpy(url_obs, argv[i + 1]);
			i++;
		} else if (!strcmp(argv[i], "--replay-path")) {
			if (argc != i + 2) die("Syntax: --replay-path needs a path...");
			...
			"Syntax: Unknown argument!\n"
			"Usage: backend --disable-obs/--url-server/--url-obs/--replay-path <url/path>",
			EXIT_FAILURE
		);
	}
	...
	printf("INFO: Server loaded!\nPress 'o' to connect to OBS Studio!");

	// TODO ASK does breaking really work here?
	while (true);

	// WebSocket stuff, again
	printf("TODO debug stmt %d\n", __COUNTER__);
	if (pthread_join(thread, NULL) != 0) fprintf(stderr, "ERROR: Failed to join thread!\n");
	printf("TODO debug stmt %d\n", __COUNTER__);

cleanup:
	printf("TODO debug stmt %d\n", __COUNTER__);
	mg_mgr_free(&mgr_svr); // TODO NOW DEBUG segfault
	printf("TODO debug stmt %d\n", __COUNTER__);
	mg_mgr_free(&mgr_obs);
	printf("TODO debug stmt %d\n", __COUNTER__);
	free(url_obs);
